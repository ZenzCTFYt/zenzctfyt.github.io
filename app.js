...
function isMax1080pResolution(width, height) {
  ...
  // Maksimal 1080p:
  // Landscape sampai 3840x2160
  // Portrait sampai 2160x3840
  ...
  fileInput.addEventListener('change', async (e) => {
    ...
    try {
      ...
      if (!isResolutionAllowed) {
        ...
        appendLog('> Maksimal resolusi yang boleh diproses adalah 4k.');
      ...
      appendLog('> Resolusi valid: maksimal 4k.');
...
if (patchBtn) {
  patchBtn.addEventListener('click', async () => {
    if (!selectedFile) {
      setStatus('> Pilih video dengan resolusi maksimal 4k terlebih dahulu.');
      return;
    }

    if (!isResolutionAllowed) {
      setStatus('> Video ditolak. Hanya resolusi maksimal 4k yang boleh diproses.');
      return;
    }
